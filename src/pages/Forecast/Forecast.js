...
function Forecast() {
  ...
  const getAllHourlyForecast = () => {
    ...
    for (let i = 0; i < forecast.list.length; i++) {
      ...
      hourlyData.push({
        ...
        day: dayLabel,
        hour24: itemHour,
        fullTime: itemDate.toLocaleTimeString("en-US", {
          hour: "numeric",
          hour12: true,
        }),
  ...
  const visibleHourlyForecast = allHourlyForecast.slice(
    hourlyStartIndex,
    hourlyStartIndex + visibleCount,
  );
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      const maxStartIndex = Math.max(
        0,
        allHourlyForecast.length - getVisibleCount(),
      );
  ...
  const avgLow = dailyForecast.length
    ? dailyForecast.reduce((sum, day) => sum + day.low, 0) /
      dailyForecast.length
    : 0;
  const avgRain = dailyForecast.length
    ? dailyForecast.reduce((sum, day) => sum + day.rain, 0) /
      dailyForecast.length
    : 0;
            ...
            <>
              ...
              <div className="daily-forecast">
                {dailyForecast.map((day, index) => (
                  <div
                    key={index}
                    className={`forecast-card ${day.isToday ? "today-card" : ""}`}
                  >
                    <div className="forecast-header">
                      ...
                      {day.isToday && (
                        <div className="today-badge">Current Day</div>
                      )}
                    </div>
            ...
            <>
              ...
              <div className="hourly-navigation">
                ...
                <div className="hourly-range">
                  <span className="range-info">
                    Showing {hourlyStartIndex + 1} -{" "}
                    {Math.min(
                      hourlyStartIndex + visibleCount,
                      allHourlyForecast.length,
                    )}{" "}
                    of {allHourlyForecast.length} forecasts
                  </span>
                ...
                <button
                  ...
                  onClick={handleNext}
                  disabled={
                    hourlyStartIndex + visibleCount >= allHourlyForecast.length
                  }
                >
                  <span>Next</span>
              ...
              <div className="hourly-forecast" ref={scrollContainerRef}>
                {visibleHourlyForecast.map((hour, index) => {
                  const cardType = hour.isNow
                    ? "now-card"
                    : hour.isToday
                      ? "today-hour-card"
                      : "";
                  return (
                    <div key={index} className={`hour-card ${cardType}`}>
                      <div
                        className={`hour-time ${hour.isNow ? "now-time" : ""}`}
                      >
                        {hour.time}
                        {hour.day && (
                      ...
                      {hour.isToday && !hour.isNow && (
                        <div className="today-hour-badge">Today</div>
                      )}
                      ...
                      <div
                        className={`hour-temp ${hour.isNow ? "now-temp" : ""}`}
                      >
              ...
              <div className="weekly-summary">
                ...
                <div className="week-bars">
                  {dailyForecast.map((day, index) => (
                    <div
                      key={index}
                      className={`week-bar-item ${day.isToday ? "today-week-item" : ""}`}
                    >
                      <div className="week-day">
                        {day.shortDay || day.day.substring(0, 3)}
                        {day.isToday && (
                          <span className="today-week-badge">Today</span>
                        )}
                      </div>
